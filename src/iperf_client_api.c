...
iperf_connect(struct iperf_test *test)
{
    ...
    if (test->ctrl_sck < 0) {
        ...
        if (test->ctrl_hostname != (char*) 0) {
                test->ctrl_sck = netdial(test->settings->domain, Ptcp, test->bind_address, 0, test->ctrl_hostname, test->server_port, test->settings->connect_timeout);
        } else {
